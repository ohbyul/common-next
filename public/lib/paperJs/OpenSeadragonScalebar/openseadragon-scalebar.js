...
(function($) {
    ...
    $.Scalebar.prototype = {
        ...
        refresh: function(options) {
            ...
            var props = this.sizeAndTextRenderer(currentPPM, this.minWidth, this.unit);
        ...
        getScalebarLocation: function() {
            ...
            if (this.location === $.ScalebarLocation.CUSTOM_BOTTOM_RIGHT) {
                var barWidth = this.divElt.offsetWidth;
                var barHeight = this.divElt.offsetHeight;
                var container = this.viewer.container;
                var x = container.offsetWidth - barWidth;
    ...
    $.ScalebarSizeAndTextRenderer = {
         ...
         */
        METRIC_LENGTH: function(ppm, minSize, unit) {
            return getScalebarSizeAndTextForMetric(ppm, minSize, "m", unit);
        },
        /**
    ...
    function getScalebarSizeAndTextForMetric(ppm, minSize, unitSuffix, unit) {
        var value = normalize(ppm, minSize);
        var factor = roundSignificand(value / ppm * minSize, 3);
        var size = value * minSize;
        var valueWithUnit = getWithUnit(factor, unitSuffix, unit);
        return {
            size: size,
            text: valueWithUnit
        };
    ...
    function getWithUnit(value, unitSuffix, unit) {
        if (unit == 'px') {
            return value + ' px';
        }
        else {
            ...
            if (value < 0.001) {
                return value * 1000000 + " &mu;" + unitSuffix; //µ
            }
            if (value < 1) {
