...
export const onlyNumGlobal = /[^0-9]/gi                     //숫자-전역검색-대소문자구분X
export const NotPrecedeHyphen = /(\-{1,2})$/g               //선행으로 보여지는 하이픈 재거

//검증용
export const regex_UserId = /^[a-z0-9_-]{5,20}$/                    //영문으로 시작하고 영문+숫자 최대 4~8자
export const regex_LoginId = /^[a-z]+[a-z0-9]{5,9}$/                //영문으로 시작하고 영문+숫자 최대 6~10자
...
export const regex_organization_number = /^(\d{0,3})(\d{0,2})(\d{0,5})$/g //사업자번호 자릿수 하이픈
...
// 최소 8 자, 최소 하나의 문자 + 하나의 숫자 (특수문자X)
// export const regex_pwd = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/

// 영문, 숫자, 특수문자 중 2가지 이상 조합하여 10자리 이내의 암호 정규식 (2가지 조합)
export const regex_pwd = /^(?=.*[a-zA-Z])(?=.*[\d@$!%*?&#.~_-])[A-Za-z\d@$!%*?&#.~_-]{8,15}$/
