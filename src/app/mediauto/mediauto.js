...
import { annotationLoad } from './tools/save.js'
...
let Mediauto = function mediauto() {
    ...
    var me = {
        ...
        */
        ...
        selectRegion: function (reg) {
            ...
            for (i = 0; i < me.objInfo.regions.length; i += 1) {
                if (me.objInfo.regions[i] === reg) {
                    // if (Mediauto.latestWork == "Annotation" && Mediauto.latestStatus == 'progressing') {
                    //     fullySelected = true;
                    // }
                    // else if (Mediauto.latestWork == "Review") {
                    //     if (reg.review_yn == "Y") {
                    //         // fullySelected = false;
                    //     }
                    //     else {
                    ...
                    // else if (Mediauto.latestWork == "Termination") {
        ...
        selectComment: function (reg) {
            ...
            if (type === 'area') {
                for (let seg of Mediauto.region.path.segments) {
        ...
        newRegion: function (arg, imageNumber, isDrawing) {
            ...
            if (isDrawing) {
                ...
                var size = 10 / Mediauto.viewer.viewport.getZoom();
                if (size > 12) {
                    size = 13;
                }
        ...
        newPoint: function (point, hex) {
            ...
            if (!Mediauto.region.arrPath) {
                Mediauto.region.arrPath = [];
            }
            Mediauto.region.arrPath.push(path)
        },
        ...
        newMeasurement: function (arg) {
            ...
            if (arg.path) {
                reg.path = arg.path;
                reg.path.strokeWidth = Mediauto.dom.querySelector('#strokeDiv .slider').slider('value');
                reg.path.strokeColor = 'rgba(' + (arg.hex ? me.hexToRgb(arg.hex) : '0, 0, 80') + ',' + 1 + ')';
                reg.path.strokeScaling = false;
        ...
        newCommentArea: function (arg) {
            ...
            if (arg.path) {
                ...
                reg.path.strokeScaling = false;
                // reg.path.fillColor = 'rgba(' + (arg.hex? me.hexToRgb( arg.hex) : '0, 0, 80') + ',' + 0.1+ ')';
                reg.path.fillColor = Mediauto.color.commentArea
                reg.path.selected = false;
            }
        ...
        newCommentBox: function (arg, imageNumber, contents) {
            ...
            if (arg.path) {
                ...
                reg.path.strokeColor = 'rgba(' + (arg.hex ? me.hexToRgb(arg.hex) : '0, 0, 80') + ',' + 1 + ')';
                reg.path.strokeScaling = false;
                reg.path.fillColor = Mediauto.color.commentBox;
                reg.path.selected = false;
            ...
            if (!Mediauto.dom.querySelector(`#div${reg.name}`).length) {
                ...
                var textarea = $(`<div class='comment-wrapper' id='div${reg.name}' name='${reg.name}'` +
                    ...
                    `<div onblur="Mediauto.dbCommentSave()" contenteditable='true' style='width: 100%; height: 100%; padding: 4px; overflow-y: scroll;'>${contents.replace(/\n/g, '<br>')}` +
                    "</div></div>");
                ...
                Mediauto.dom.querySelector('.openseadragon-container').append(textarea);

                // $($('#content')[0].shadowRoot.querySelector(`#div${reg.name}`)).draggable({
                //     disabled: true
                // });

                Mediauto.dom.querySelector(`#div${reg.name}`).draggable({
                    // handle: '.drag-btn',
                    // containment: "parent"
                })
                // }).resizable({minWidth: 200, minHeight:100, handles: 'all', containment: "parent"  });

                Mediauto.dom.querySelector(`#div${reg.name}`).on('click', function (e) {
                    ...
                    for (let div of Mediauto.dom.querySelectorAll('.comment-wrapper')) {
                        $(div).css('z-index', 3);
                    }
                    $(this).css('z-index', '9');
                    me.tools['select'].mouseDown(paper.view.viewToProject(new paper.Point(e.clientX - $('#main-lnb').width(), e.clientY - $('.topMenuArea').height())))
                    Mediauto.objComment?.box.path.bringToFront()
                })

                Mediauto.dom.querySelector(`#div${reg.name}`).on('dblclick', function () {
                ...
                Mediauto.dom.querySelector(`#div${reg.name}`).on('dragstart', function (e) {
                ...
                Mediauto.dom.querySelector(`#div${reg.name}`).on('drag', function (e) {
                ...
                Mediauto.dom.querySelector(`#div${reg.name}`).on('dragstop', function (e) {
        ...
        newCommentLine: function (commentArea, commentBox, arg, imageNumber, isDrawing) {
            ...
            if (arg.path) {
                ...
                reg.path.strokeColor = 'rgba(' + (arg.hex ? me.hexToRgb(arg.hex) : '0, 0, 80') + ',' + 1 + ')';
                reg.path.strokeScaling = false;
                reg.path.fillColor = 'rgba(' + (arg.hex ? me.hexToRgb(arg.hex) : '0, 0, 80') + ',' + Mediauto.dom.querySelector('#opacityDiv .slider').slider('value') + ')';
                reg.path.selected = false;
        ...
        newText: function (reg, fontSize, visible, text) {
            ...
            //   reg.text.rotate(Number(Mediauto.viewer.viewport.degrees) * -1)
            reg.text.sendToBack();
        },
        ...
        initText: function (reg) {
            ...
            // Mediauto.region.text.setFontSize(Mediauto.ImageInfo[1].Regions.length > 0? Mediauto.ImageInfo[1].Regions[0].text.fontSize : 20);
            // reg.text.position = reg.path.position;
        },
        ...
        removeComment: function (imageNumber) {
            ...
            if ($(Mediauto.dom.querySelector(`#div${me.objComment.name} div`)).is(':focus')) {
                return;
            }
            ...
            axios.delete(web_url + `/api/annotation/comments/${me.objComment.uid}`).then((result) => {
                ...
                Mediauto.dom.querySelector(`#div${me.objComment.name}`).remove();
        ...
        pressHandler: function (event) {
                ...
                for (let div of arrDiv) {
                    ...
                    for (let reg of me.objInfo.comments) {
                        if (`div${reg.box.name}` == commentId) {
                            reg.box.text.setContent(Mediauto.dom.querySelector(`#div${reg.box.name} div`).innerText);
                            // reg.box.text.setContent(Mediauto.dom.querySelector(`#div${reg.box.name}`).value);
                            // Mediauto.dom.querySelector('.comment-wrapper').remove();
                            break;
                        }
                    }
                }
            ...
            if (me.key == 'ctrl' || me.selectedTool == "move") {
                ...
                // if( hitResult == null && me.drawingPolygonFlag == false && Mediauto.drawingPointFlag == false && Mediauto.selectedTool.indexOf("region") == -1) {
                //     if (me.region) {
                ...
                //     if (Mediauto.selectedTool.indexOf("draw") == -1) {
                //         if (Mediauto.key != '') {
                //             me.navEnabled = false;
                //         }
                //         else {
        ...
        dragHandler: function (event) {
            ...
            if (me.isLocked && me.selectedTool == 'select') {
                if (!Mediauto.region?.path.type.includes('COMMENT')) {
                    return;
                }
            }
        ...
        moveHandler: function (event) {
            ...
            let viewportPoint = Mediauto.viewer.viewport.pointFromPixel(event.position);
            let imagePoint = Mediauto.viewer.viewport.viewportToImageCoordinates(viewportPoint.x, viewportPoint.y);
        ...
        mouseDown: function (x, y) {
            ...
            if (me.selectedTool == 'region' && !me.isLocked) {
                if (!Mediauto.region) return false;
                if (Mediauto.region.path.type == 'LINE' || Mediauto.region.path.type == 'BBOX' || Mediauto.region.path.type == 'MEASUREMENT') return false;
                if (Mediauto.region.arrPath) return false;
                ...
                let hitResult = Mediauto.region.path.hitTest(point, {
                    tolerance: 0,
                    stroke: false,
                    segments: true,
                    fill: true,
                ...
                if (Mediauto.region.path == hitResult?.item) {
        ...
        mouseDrag: function (x, y, dx, dy, event) {
            ...
            if (me.handle) {
                ...
                if (Mediauto.region.path.type == "BBOX") {
                    // if (Mediauto.region.path.type != null) {
                    ...
                    if (Mediauto.region.path.type == "MEASUREMENT") {
                    ...
                    else if (Mediauto.region.path.type.includes("COMMENT")) {
                        if (Mediauto.region.path.type != 'COMMENTBOX') {
                            me.handle.x += point.x - me.handle.point.x;
                            me.handle.y += point.y - me.handle.point.y;
                            me.handle.point = point;
                        }
                        ...
                        if (Mediauto.region.path.type == 'POINT' && Mediauto.region.arrPath) {
                            Mediauto.region.arrPath[me.handle.index].position.x = me.handle.x;
                            Mediauto.region.arrPath[me.handle.index].position.y = me.handle.y;
                        }
                ...
                if (Mediauto.key == 'r' || me.selectedTool == "rotate") {
                    if (Mediauto.region != null && Mediauto.region.path.type != "BBOX") {
                        dpoint.x *= Mediauto.viewer.viewport.getZoom();
                        dpoint.y *= Mediauto.viewer.viewport.getZoom();
                    ...
                    if (me.selectedTool != "drawMeasurement" && Mediauto.region?.path.type != 'MEASUREMENT') {
                        me.chkSaved(false);
                    }
                }
        ...
        resizeFontSize: function () {
            let reg;
            let size = 10 / Mediauto.viewer.viewport.getZoom();
            for (var regCnt = 0; regCnt < Mediauto.objInfo.regions.length; regCnt++) {
                reg = Mediauto.objInfo.regions[regCnt];
                ...
                if (size > 13) {
                    ...
                    reg.text.setFontSize(10 / Mediauto.viewer.viewport.getZoom())
                }

                reg.text.position = reg.path.position;
            }

            for (var regCnt = 0; regCnt < Mediauto.objInfo.measurements.length; regCnt++) {
                reg = Mediauto.objInfo.measurements[regCnt];
            ...
            for (var regCnt = 0; regCnt < Mediauto.objInfo.comments.length; regCnt++) {
                reg = Mediauto.objInfo.comments[regCnt];
                ...
                if (size > 13) {
                    ...
                    area.text.setFontSize(10 / Mediauto.viewer.viewport.getZoom())
                }
        ...
        resizeCommentBox: function () {
            ...
            let arrComments = Mediauto.objInfo.comments;
            for (let obj of arrComments) {
                let textBox = obj.box;
                if (Mediauto.dom.querySelector(`#div${textBox.name}`)) {
                    ...
                    Mediauto.dom.querySelector(`#div${textBox.name}`).style.left = `${leftTopWebPixel.x + 1}px`
                    Mediauto.dom.querySelector(`#div${textBox.name}`).style.top = `${leftTopWebPixel.y + 1}px`
                    Mediauto.dom.querySelector(`#div${textBox.name}`).style.width = `${rightBottomWebPixel.x - leftTopWebPixel.x - 2}px`
                    Mediauto.dom.querySelector(`#div${textBox.name}`).style.height = `${rightBottomWebPixel.y - leftTopWebPixel.y - 2}px`
                }
            }
        ...
        statusSelection: function () {
            ...
            if (status == "start") {
                ...
                if (work == "Termination") {
                    ...
                    if (Mediauto.statusInfo.info_yn == "N") {
                ...
                if (!me.isLocked || _auth_level == 'SM' || _auth_level == 'GM' || _auth_level == 'OM') {
                    //Clinical Info가 입력되지 않은 경우 termination 불가
                    if (work == "Termination" && Mediauto.statusInfo.info_yn == "N") {
                        customAlert('Clinical Info has not been entered.')
                        return;
                    }
        ...
        statusUpdate: function (work, status) {
            ...
            var param = {
                slide_id: Mediauto.slide_id || 908,
                work: work,
                status: status
            }
        ...
        objectListPopup: function () {
            ...
            if (Mediauto.latestWork == "Annotation") {
            ...
            if (!adminFlag && (!ownFlag || _auth_level == "UO")) {
                Mediauto.latestWork = "Object List";
                Mediauto.latestStatus = "";
            }

            // 매번 내용 전부 지우고 다시 그려줌
            me.dom.querySelector("#popup-object .titleText").innerText = 'Annotation';
            ...
            for (var regCnt = 0; regCnt < me.objInfo.regions.length; regCnt++) {
                ...
                if (reg.del_yn == "N" && reg.path.type != 'MEASUREMENT') {
                    ...
                    element_div.addEventListener("click", function () {
                        if (me.tools[Mediauto.selectedTool] && me.tools[Mediauto.selectedTool].onDeselect) {
                            me.tools[Mediauto.selectedTool].onDeselect();
                        }
                        ...
                        for (var regCnt = 0; regCnt < me.objInfo.regions.length; regCnt++) {
                            ...
                            if (reg.name == this.innerText) {
                                ...
                                Mediauto.viewer.viewport.centerSpringX.target.value = reg.path.position.x / 1000;
                                Mediauto.viewer.viewport.centerSpringY.target.value = reg.path.position.y / 1000;
                            }
                            else {
                    ...
                    if ((Mediauto.latestWork == "Review" || Mediauto.latestWork == "Termination") && ownFlag && Mediauto.latestStatus != 'complete') {
                        element_div_accept = document.createElement('div');
                        if (Mediauto.latestWork == "Review") {
                            if (reg.review_yn == 'Y') {
                        ...
                        element_div_accept.addEventListener("click", function () {
                            ...
                            for (var regCnt = 0; regCnt < me.objInfo.regions.length; regCnt++) {
                                ...
                                if (reg.name == this.previousSibling.innerText) {
                                    ...
                                    if (ownFlag && Mediauto.latestStatus != 'complete') {
                    ...
                    if (!Mediauto.isLocked && _auth_level != "UO" && ownFlag && Mediauto.latestStatus != "complete") {
                        if (Mediauto.latestWork == "Review") {
                            if (reg.review_yn != 'Y') {
                    ...
                    element_div_label.addEventListener("click", async function (event) {
                        if (me.tools[Mediauto.selectedTool] && me.tools[Mediauto.selectedTool].onDeselect) {
                            await me.tools[Mediauto.selectedTool].onDeselect();
                        }
                        ...
                        let parent_object = Mediauto.dom.getElementById(`${this.parentElement.parentElement.id}`);
                        ...
                        if (Mediauto.latestWork == "Review") {
                            if (Mediauto.region.review_yn == 'Y') {
                                me.dom.querySelector("#changeLabelDiv").style.visibility = 'hidden';
                                return;
                            }
                        }
                        else if (Mediauto.latestWork == "Termination") {
                            if (Mediauto.region.termination_yn == 'Y') {
                                me.dom.querySelector("#changeLabelDiv").style.visibility = 'hidden';
                                return;
                            }
                        }
                        ...
                        var changeLabelDivTop =
                            Mediauto.dom.querySelectorAll(`#changeLabelDiv`).offsetParent.offsetTop
                            + Mediauto.dom.querySelectorAll(`#changeLabelDiv`).offsetTop;

                        // var changeLabelDivOriginHeight = Mediauto.dom.querySelectorAll(`#changeLabelDiv`).attr('origin-height');
                        // 좌표 type에 따라 갯수가 다를것 대비하여 직접 계산
                        var changeLabelDivOriginHeight =
                            parseInt(Mediauto.dom.querySelector(`#changeLabelDiv`).css('border-top'))
                            + parseInt(Mediauto.dom.querySelector(`#changeLabelDiv`).css('border-bottom'))
                            + parseInt(Mediauto.dom.querySelector(`#changeLabelDiv`).css('padding-top'))
                            + parseInt(Mediauto.dom.querySelector(`#changeLabelDiv`).css('padding-bottom'))
                            + (
                                Mediauto.dom.querySelectorAll(`#changeLabelDiv li`).length
                                * parseInt(Mediauto.dom.querySelector(`#changeLabelDiv li`).css('height'))
                            );
                        var bodyHeight = Mediauto.dom.querySelectorAll(`body`).offsetHeight

                        if (bodyHeight - changeLabelDivTop - changeLabelDivOriginHeight - 5 < 1) {
                            Mediauto.dom.querySelectorAll(`#changeLabelDiv`).style.height = `${bodyHeight - changeLabelDivTop - 5}px`
                        }
                        else {
                            Mediauto.dom.querySelectorAll(`#changeLabelDiv`).style.height = `${changeLabelDivOriginHeight}px`;
                        }
                    });
                    ...
                    if (!reg.uid.toString().includes('temp') && !Mediauto.isLocked && _auth_level != "UO" && ownFlag && Mediauto.latestStatus != "complete") {
                        if (Mediauto.latestWork == "Review") {
                            if (reg.review_yn == 'Y') {
                                btnChangeFlag = false;
                            }
                        ...
                        if (btnChangeFlag) {
                            ...
                            element_div_change.addEventListener("click", function (event) {
                                ...
                                if (Mediauto.isLocked || !ownFlag || _auth_level == "UO") {
                                    return;
                                }
                                ...
                                if (Mediauto.region !== null) {
                                    if (Mediauto.region.labelNo !== null) {
                                        // 기존 주석을 change 버튼 클릭시 update
                                        var param = {};
                                        var values = [];

                                        values.push({
                                            annotation_id: Mediauto.region.uid,
                                            label_id: Mediauto.region.labelNo
                                        })
            ...
            if (Mediauto.latestWork != "Object List" || adminFlag) {
                ...
                if ((ownFlag && Mediauto.latestStatus != 'complete')) {
                    ...
                    element_div_complete.addEventListener("click", function () {
                        ...
                        $.confirm({
                            title: '',
                            content: 'Are you sure you want to finish ' + Mediauto.latestWork + ' work?',
                            type: 'custom',
                            typeAnimated: true,
                            animation: 'none',
                            buttons: {
                                tryAgain: {
                                    ...
                                    action: () => {
                                        ...
                                        if (completeFlag) {
                                            ...
                                            me.statusUpdate(Mediauto.latestWork, 'complete');
                                            me.loadDBData();
                                        }
                                        else {
                ...
                if (ownFlag || adminFlag) {
                    ...
                    element_div_reset.addEventListener("click", function () {
                        ...
                        $.confirm({
                            title: '',
                            content: 'Are you sure you want to reset ' + Mediauto.latestWork + ' data?',
                            type: 'custom',
                            typeAnimated: true,
                            animation: 'none',
                            buttons: {
                                tryAgain: {
                                    ...
                                    action: () => {
                                        ...
                                        var param = {
                                            slide_id: Mediauto.slide_id,
                                            work: Mediauto.latestWork
                                        }

                                        if (_auth_level != "UO") {
                                            axios.post(web_url + `/api/annotation/status-reset`, param)
                                                .then((data) => {
                                                    // toast.success(Mediauto.latestWork + " reset success!");
                                                    me.chkSaved(true);
                    ...
                    element_div_cancel.addEventListener("click", function () {
                        ...
                        $.confirm({
                            title: '',
                            content: 'Are you sure you want to cancel the ' + Mediauto.latestWork + ' role?',
                            type: 'custom',
                            typeAnimated: true,
                            animation: 'none',
                            buttons: {
                                    ...
                                    action: () => {
                                        ...
                                        if (_auth_level != "UO") {
                                            axios.post(web_url + `/api/annotation/status-cancel`, param)
                                                .then((data) => {
                                                    // toast.success(Mediauto.latestWork + " cancel success!");
                                                    me.chkSaved(true);
                    ...
                    if (me.initPopupFlag) {
                        if (me.dom.querySelector(".btnSave") !== null) {
                            ...
                            if (Mediauto.latestWork == 'Object List') {
            ...
            if (Mediauto.latestWork == "Object List" || (!adminFlag && !ownFlag)) {
        ...
        dbAnnotationLoad: function () {
            me.annotationLoad = annotationLoad
            me.annotationLoad()
                .then(function (result) {
                    ...
                    for (var resultCnt = 0; resultCnt < result.length; resultCnt++) {
                        ...
                        if (result[resultCnt].coordinate_kind_cd == "BBOX") {
                            ...
                            for (let pointCnt = 0; pointCnt < point.length; pointCnt += 1) {
                                ...
                                // segments.push([point[pointCnt][0] * projectPixel.x / Mediauto.fileWidth, point[pointCnt][1] * projectPixel.y / Mediauto.fileHeight]);
                            }
                        }
                        ...
                        if (Mediauto.dom.querySelector("#labelBar div input") !== null) {
                            if (!Mediauto.dom.querySelector("#labelBar div[data-label-no='" + reg.labelNo + "'] input").checked) {
                                reg.path.visible = false;
                                reg.path.selected = false;
                                reg.text.visible = false;
                            }
                    ...
                    if (me.debug) { console.log("< MediautoDBLoad resolve success. Number of regions:", me.objInfo.regions.length); }
                    // })
                    // .catch(function(error) {
                    //     console.error('< MediautoDBLoad resolve error', error);
                    // });
                });
        },
        ...
        dbCommentSave: function () {
            ...
            let objComment = Mediauto.objComment;

            let param = {
                slideId: Mediauto.slide_id,
                ...
                contents: Mediauto.dom.querySelector(`#div${Mediauto.objComment.name} div`).innerText.replace(/\n\n/g, '\n')
            }
        ...
        dbCommentLoad: function () {
            Mediauto.dom.querySelectorAll(`.comment-wrapper`).remove()

            me.commentSeq = 1;
            axios.get(web_url + `/api/annotation/comments/${Mediauto.slide_id}`)
                .then((result) => {
                    var result = result.data
                    ...
                    for (var resultCnt = 0; resultCnt < result.length; resultCnt++) {
                        ...
                        path.fillColor = 'rgba(' + Mediauto.hexToRgb('0, 0, 80') + ',' + 0.1 + ')'
                        path = Mediauto.newCircle({ path: path }, 1, true).path;
                        regArea = Mediauto.newCommentArea({ path: path }, 1, true);
                        ...
                        regBox = Mediauto.newCommentBox({ path: path }, 1, result[resultCnt].contents);
                        ...
                        regLine = Mediauto.newCommentLine(regArea, regBox, { path: path }, 1, '');

                        Mediauto.objInfo.comments.push({ uid: result[resultCnt].comment_id, name: `Comment${Mediauto.commentSeq}`, area: regArea, box: regBox, line: regLine });
        ...
        dbStatusLoad: async function () {
            Mediauto.slide_id = 908
            let params = {
                page: 1,
                pageLength: 1000,
                whereOptions: ([] = [{ where_key: 'ts.id', where_value: Mediauto.slide_id || 908, where_type: 'AND' }]),
                orderOptions: ([] = []),
            };
            me.initPopupFlag = true;

            await axios.post(web_url + `/annotation/image-status-list`, params)
                .then((result) => {
                    ...
                    if (me.isLocked) {
                        $(Mediauto.dom.querySelector('#tools-side')).find('.selected').removeClass('selected')
                        $(Mediauto.dom.querySelector('#tools-side')).find('#select').addClass('selected')
                        me.selectedTool = 'select';
                        me.prevTool = 'select';
                        me.initCursor();
                    }
        ...
        dbLabelLoad: function () {
            let params = {
                page: 1,
                pageLength: 1000,
                whereOptions: ([] = [{ where_key: 'tumor_cd', where_value: Mediauto.tumor_code, where_type: 'AND' }]),
            };

            axios.post(web_url + `/annotation/label-list`, params)
                .then((result) => {
                    console.log(result)
                    ...
                    labelList = [
                        {
                          "label_id": 4913,
                          "label_nm": "LN_normal",
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Lymph node, normal",
                          "rgb_hex": "0x06e49c",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                          "modify_utc_dtm": "2023-09-13T07:53:38.000Z",
                          "modify_ssid": "UU101"
                        },
                        {
                          "label_id": 4402,
                          "label_nm": "LN_normal",
                          "coordinate_kind_cd": "LINE",
                          "tumor_cd": "BRCA",
                          "label_desc": "Lymph node, normal",
                          "rgb_hex": "0x06e49c",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                          "modify_utc_dtm": "2023-09-13T07:53:32.000Z",
                          "modify_ssid": "UU101"
                        },
                        {
                          "label_id": 3891,
                          "label_nm": "LN_normal",
                          "coordinate_kind_cd": "BBOX",
                          "tumor_cd": "BRCA",
                          "label_desc": "Lymph node, normal",
                          "rgb_hex": "0x06e49c",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                          "modify_utc_dtm": "2023-09-13T07:53:25.000Z",
                          "modify_ssid": "UU101"
                        },
                        {
                          "label_id": 1732,
                          "label_nm": "LN_normal",
                          "coordinate_kind_cd": "SEG",
                          "tumor_cd": "BRCA",
                          "label_desc": "Lymph node, normal",
                          "rgb_hex": "0x06e49c",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                          "modify_utc_dtm": "2023-11-06T02:22:24.000Z",
                          "modify_ssid": "UU101"
                        },
                        {
                          "label_id": 5183,
                          "label_nm": "LN_malignant",
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Metastatic, lymh node, malignant444",
                          "rgb_hex": "0x0e9258",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:19.000Z",
                          "create_ssid": "UU101",
                          "modify_utc_dtm": "2023-09-13T07:53:38.000Z",
                          "modify_ssid": "UU101"
                        },
                        {
                          ...
                          "coordinate_kind_cd": "LINE",
                          "tumor_cd": "BRCA",
                          "label_desc": "Metastatic, lymh node, malignant333",
                          "rgb_hex": "0x0e9258",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:19.000Z",
                          "create_ssid": "UU101",
                        ...
                        },
                        {
                          ...
                          "coordinate_kind_cd": "SEG",
                          "tumor_cd": "BRCA",
                          "label_desc": "Metastatic, lymh node, malignant111",
                          "rgb_hex": "0x0e9258",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:19.000Z",
                          "create_ssid": "UU101",
                          "modify_utc_dtm": "2023-09-13T07:53:18.000Z",
                          "modify_ssid": "UU101"
                        },
                        {
                          "label_id": 4914,
                        ...
                        {
                          "label_id": 4905,
                          "label_nm": "NT_inflammation",
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Nontumor tissue, inflammation",
                          "rgb_hex": "0xcbaa33",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                        ...
                        },
                        {
                          ...
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Nontumor tissue, normal",
                          "rgb_hex": "0xbb05e5",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:19.000Z",
                          "create_ssid": "UU101",
                        ...
                        },
                        {
                          ...
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Nontumor tissue, other",
                          "rgb_hex": "0x5419a3",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                        ...
                        },
                        {
                          ...
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Tumor, primary, benign, atypical ductal hyperplasia",
                          "rgb_hex": "0xe2117e",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                        ...
                        {
                          "label_id": 4910,
                          "label_nm": "TP_ALH/LCIS",
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Tumor, primary, atypical lobular hyperplasia/LCIS",
                          "rgb_hex": "0x4e2e30",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                        ...
                        {
                          "label_id": 5180,
                          "label_nm": "TP_benign_other",
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Tumor, primary, benign, other, including adenosis and adenoma",
                          "rgb_hex": "0x899bf8",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:19.000Z",
                          "create_ssid": "UU101",
                        ...
                        },
                        {
                          ...
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Tumor, primary, dutal carcinoma in situ",
                          "rgb_hex": "0x84c4b0",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:19.000Z",
                          "create_ssid": "UU101",
                        ...
                        },
                        {
                          ...
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Tumor, primary, epithelial-myoepithelial tumors",
                          "rgb_hex": "0x6a30d8",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                        ...
                        },
                        {
                          ...
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                          "modify_utc_dtm": "2023-09-13T07:53:26.000Z",
                          "modify_ssid": "UU101"
                        },
                        {
                          "label_id": 1727,
                        ...
                        {
                          "label_id": 5182,
                          "label_nm": "TP_malignant",
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Tumor, primary, invasive breast carcinoma",
                          "rgb_hex": "0x2720cc",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:19.000Z",
                          "create_ssid": "UU101",
                        ...
                        {
                          "label_id": 4912,
                          "label_nm": "TP_necrosis",
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Tumor, primary, necrosis",
                          "rgb_hex": "0x9e3c1f",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                        ...
                        },
                        {
                          ...
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Tumor, primary, papilllary neoplasms",
                          "rgb_hex": "0x6375bf",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:16.000Z",
                          "create_ssid": "UU101",
                        ...
                        },
                        {
                          ...
                          "coordinate_kind_cd": "POINT",
                          "tumor_cd": "BRCA",
                          "label_desc": "Tumor, primary, benign, usual ductal hyperplasia",
                          "rgb_hex": "0x0ab4db",
                          "del_yn": "N",
                          "create_utc_dtm": "2022-09-30T05:31:19.000Z",
                          "create_ssid": "UU101",
        ...
        dbDeepzoomInformationLoad: async function () {
            ...
            await axios.get(deepcis_url + '/info/' + Mediauto.statusInfo.pre_path + `/` + Mediauto.statusInfo.filename)
                .then((result) => {
        ...
        dbSlideInfoLoad: async function () {
            let deepzoomCode;
            // await axios.get(web_url + `/api/tumor/detail-slide/` + Mediauto.slide_id)
            await axios.get(web_url + `/tumor/slide-info/` + 429)
                .then((result) => {
                    let deepzoomInfo = result.data.deepzoomInfo;
                    deepzoomCode = deepzoomInfo?.statusCode;

                    Mediauto.tumor_code = result.data.baseInfo.tumorCode;
                    Mediauto.slide_url = deepzoomInfo.slide_url;
                    Mediauto.jpegFilePath = result.data.jpegFilePath;

                    Mediauto.fileWidth = result.data.slideInfo.file_width;
                    Mediauto.fileHeight = result.data.slideInfo.file_height;
                    Mediauto.dom.querySelector(".infor.Txt2.width").innerText = result.data.slideInfo.file_width;
                    Mediauto.dom.querySelector(".infor.Txt2.height").innerText = result.data.slideInfo.file_height;
        ...
        dbMemoLoad: async function () {
            ...
            await axios.get(web_url + `/api/annotation/memos/${Mediauto.slide_id}`)
                .then((result) => {
                    if (result.data.length) {
                        ...
                        let memoList = Mediauto.dom.querySelector('#popup-memo .list');
        ...
        createMemoElement: function (isNew, item) {
            ...
            let divWrapper = Mediauto.dom.querySelector(`.memo-wrapper[data-memo-id="${memoId}"]`)
            if (isNew) {
        ...
        save: async function () {
            ...
            for (var regCnt = 0; regCnt < me.objInfo.regions.length; regCnt++) {
                ...
                if (Mediauto.latestWork == "Review" && reg.review_yn == "Y") {
                    continue;
                }

                if (Mediauto.latestWork == "Termination" && reg.termination_yn == "Y") {
                    continue;
                }
        ...
        coordinateInfo: function (reg) {
            ...
            for (var segCnt = 0; segCnt < segments.length; segCnt++) {
                ...
                if (!reg.path.type.includes('COMMENT')) {
                    if (imagePoint.x > Mediauto.imageSize.x) {
                        imagePoint.x = Mediauto.imageSize.x
                    }

                    if (imagePoint.y > Mediauto.imageSize.y) {
                        imagePoint.y = Mediauto.imageSize.y
                    }
        ...
        load: function () {
            ...
            var i, obj, reg;
            if (localStorage.Mediauto) {
                console.log("Loading data from localStorage");
                obj = JSON.parse(localStorage.Mediauto);
        ...
        initAnnotationOverlay: function () {
           ...
           */
            ...
            // let imageSize = new OpenSeadragon.Point(Mediauto.fileWidth, Mediauto.fileHeight);
            ...
            // let webPixel = Mediauto.viewer.viewport.getContainerSize()
            let projectPixel = paper.view.viewToProject(webPixel)

            // me.imageSize = imageSize;
            me.imageSize = new OpenSeadragon.Point(Mediauto.fileWidth, Mediauto.fileHeight);
            ...
            // ];
            //점 크기 조절
            paper.settings.handleSize = Mediauto.handleSize;
            // me.updateRegionList();
            paper.view.draw();
        ...
        labelSelector: function (isUp) {
            ...
            if (Mediauto.dom.querySelector(`#labelDiv`).style.visibility == 'visible') {
                targetDiv = 'labelDiv';
            }
            else if (Mediauto.dom.querySelector(`#labelDiv`).style.visibility == 'visible') {
                targetDiv = 'changeLabelDiv';
            }

            currFocus = Mediauto.dom.querySelector(`#${targetDiv} :focus`).length ? Mediauto.dom.querySelector(`#${targetDiv} :focus`) : Mediauto.dom.querySelector(`#${targetDiv} .selected`)
            tabIndex = currFocus.prop('tabindex') ? currFocus.prop('tabindex') : 0;

            if (isUp) {
                if (tabIndex == 0 || tabIndex == 1) {
                    tabIndex = Mediauto.dom.querySelector(`#${targetDiv} li`).length;
                }
                else {
                    tabIndex = tabIndex - 1;
                }
                ...
                if (tabIndex == Mediauto.dom.querySelector(`#${targetDiv} li`).length) {
                    tabIndex = 1;
                }
                else {
                    tabIndex = tabIndex + 1;
            ...
            Mediauto.dom.querySelector(`#${targetDiv} li[tabindex=${tabIndex}]`).focus();
         ...
         * @desc Load general Mediauto configuration
                ...
                Promise.all([
                    ...
                    .then(() => {
                        for (let item of tools) {
                            Object.assign(Mediauto.tools, item)
                        }
                        window.Mediauto = Mediauto;
        ...
        updateFilters: function () {
            var filters = [
                OpenSeadragon.Filters.BRIGHTNESS(Mediauto.dom.querySelector('#brightnessDiv .slider').slider("value")),
                OpenSeadragon.Filters.CONTRAST(Mediauto.dom.querySelector('#contrastDiv .slider').slider("value"))
            ]
        ...
        /**
         * @function initMediauto
         * @returns {void}
         */
        initMediauto: async () => {

            if (me.debug) {
                console.log("> initMediauto promise");
            }
            // 브라우저 새로고침, 닫힐 때
            ...
            me.dom.querySelector('#btnResetFilters').addEventListener('click',
                e => {
                    Mediauto.dom.querySelector('#brightnessDiv .slider').slider("value", 0);
                    Mediauto.dom.querySelector('#contrastDiv .slider').slider("value", 1);
            ...
            var observer = new MutationObserver(function (mutations) {
                mutations.forEach(function (mutationRecord) {
                    Mediauto.dom.querySelector('#brightnessDiv .tooltip').css("left", "" + (Mediauto.dom.querySelector('#brightnessDiv span').offsetLeft - 5) + "px")
                });
            ...
            Mediauto.slide_id = 908 //urlParams.get('trId');
        ...
        addCursor: function (classNm) {
            Mediauto.dom.querySelectorAll('.openseadragon-canvas canvas')[0].classList.add(classNm);
        },
        ...
        updateCursor: function () {
            ...
            var tool = Mediauto.selectedTool;
        ...
        currentRegions: function (unscrollFlag) {
            Mediauto.updateLabelBar();
            if (me.dom.querySelector("#popup-object").style.visibility == 'visible') {
                Mediauto.objectListPopup();
            }

            //unscrollFlag true로 넘겨주면 scrolltop 동작 안함 
            Mediauto.selectedRegInfo(unscrollFlag);
        ...
        selectedRegInfo: function (unscrollFlag) {
            ...
            for (var regCnt = 0; regCnt < me.objInfo.regions.length; regCnt++) {
                ...
                if (reg.path.selected && !unscrollFlag) {
                    Mediauto.dom.querySelector('#popup-object .list').scrollTo((regCnt - del_cnt) * 65, 0)
                }
            }
        ...
        updateLatestStatus: function () {
            ...
            if (Mediauto.statusInfo.termination_yn == "Y") {
                Mediauto.latestWork = "Termination";
                Mediauto.latestStatus = "complete";
            }
            else if (Mediauto.statusInfo.termination_ssid !== null && Mediauto.statusInfo.termination_utc_dtm === null) {
                Mediauto.latestWork = "Termination";
                Mediauto.latestStatus = "progressing";
            }
            else if (Mediauto.statusInfo.review_yn == "Y") {
                Mediauto.latestWork = "Review";
                Mediauto.latestStatus = "complete";
            }
            else if (Mediauto.statusInfo.review_ssid !== null && Mediauto.statusInfo.review_utc_dtm === null) {
                Mediauto.latestWork = "Review";
                Mediauto.latestStatus = "progressing";
            }
            else if (Mediauto.statusInfo.annotation_yn == "Y") {
                Mediauto.latestWork = "Annotation";
                Mediauto.latestStatus = "complete";
            }
            else if (Mediauto.statusInfo.annotation_ssid !== null && Mediauto.statusInfo.annotation_utc_dtm === null) {
                Mediauto.latestWork = "Annotation";
                Mediauto.latestStatus = "progressing";
            }
            else {
        ...
        listUpLabelByCoordinateKindCd: function (coordinate_kind_cd, targetDiv) {
            if (me.dom.querySelector(`${targetDiv} ul`))
                    ...
                    element.addEventListener("keydown", function (e) {
                        if (e.keyCode == 13) {
                            ...
                            if (me.region.path.type == 'POINT') {
                                ...
                                me.region.path.fillColor = 'rgba(' + me.hexToRgb(this.getAttribute('data-hex') ? this.getAttribute('data-hex') : '0, 0, 80') + ',' + Mediauto.dom.querySelector('#opacityDiv .slider').slider('value') + ')';
                            }
        ...
        initOpenSeadragon: function () {
            ...
            me.viewer = new OpenSeadragon({
                ...
                // tileSources: [],
                tileSources: {
                    tileSource: 'http://localhost:5000/info/CODIPAI-BRCA-KA-00005-S-LN-01.tiff'
            ...
            //     minWidth:'150px',
            //     pixelsPerMeter: 1000000 / (Mediauto.fileWidth),
            //     color:'black',
            //     fontColor:'black',
            ...
            me.canvasOrigin = Mediauto.dom.querySelector('#openseadragon1 canvas').getContext('2d', { willReadFrequently: true })
            me.canvasZoom = Mediauto.dom.querySelector('#test1').getContext('2d', { willReadFrequently: true })
            me.canvasTemp = Mediauto.dom.querySelector('#test2').getContext('2d', { willReadFrequently: true })
            ...
            me.loadConfiguration()
                .then(async function () {
                    if (me.config.useDatabase) {
                        Promise.all([]) // [MediautoDBIP(), MyLoginWidget.init()]
                            .then(function () {
...
function resizeAnnotationOverlay() {
    if (Mediauto.debug > 1) { console.log("> resizeAnnotationOverlay"); }

    var width = Mediauto.dom.querySelector("#paperjs-container").offsetWidth;
    var height = Mediauto.dom.querySelector("#paperjs-container").offsetHeight;
    // Mediauto.dom.querySelector("canvas.overlay").offsetWidth = width;
    // Mediauto.dom.querySelector("canvas.overlay").offsetHeight = height;
    paper.view.viewSize = [
        width,
        height
    ];

    relocatePopup(Mediauto.dom.querySelector("#popup-info"), width, height);
    relocatePopup(Mediauto.dom.querySelector("#popup-object"), width, height);
    relocatePopup(Mediauto.dom.querySelector("#popup-memo"), width, height);
    relocatePopup(Mediauto.dom.querySelector("#popup-hint"), width, height);
...
function setPopupHeight() {
    if (Mediauto.dom.querySelector("#popup-object").style.height != '') {
        Mediauto.dom.querySelector("#popup-object .viewArea .list").style.height = '100%'
    }
}
...
function setSettingCookie(value) {
    ...
    document.cookie = Mediauto.settingCookie + '=' + JSON.stringify({ ...prevValue, ...value }) + `; expires=${expired.toUTCString()}; path=/`;
    Mediauto.initSetting = getSettingCookie();
};

function getSettingCookie() {
    var value = document.cookie.match('(^|;) ?' + Mediauto.settingCookie + '=([^;]*)(;|$)');
    return value ? JSON.parse(decodeURIComponent(value[2])) : Mediauto.initSetting;
};

function hrefByImageListPopup(e) {
    Mediauto.unsavedAlert(e, true);
}
